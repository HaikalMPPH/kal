...
#define dyn_array_size(arr) dyn_array_header((arr))->size
#define dyn_array_capacity(arr) dyn_array_header((arr))->capacity
...
dyn_array_ensure_capacity(void* array, usize value_size) {
    if (dyn_array_header(array)->size >= dyn_array_header(array)->capacity - 1) {
        ...
        memcpy(newArray, array, (dyn_array_size(array)) * value_size);
